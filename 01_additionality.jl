...
# define the energy carrier for hydrogen production
EnergyCarrier = "Power"
...
@constraint(ESM, RenewablePowerForHydrogen[y in year, f in fuels; f == "Power"],
    sum(FuelProductionByTechnology[y,r,h,t,f]
        for r in regions, h in hour
...
@constraint(ESM, NewElectrolyzeRequiresNewRE[y in year, f in fuels; f == "Power"],
            sum(NewCapacity[y,r,t] * CapacityFactor[r,h,t] * OutputRatio[t,f] * 8760/n_hour
                for r in regions
                for h in hour
                for t in technologies if TagRenewable[t] == 1) >=
            sum(FuelUseByTechnology[y,r,h,t,f] - InputRatio[t,f] *
            (AccumulatedCapacity[y,r,t] - NewCapacity[y,r,t]) * CapacityFactor[r,h,t] * 8760/n_hour #Stromverbrauch der neuen Elektrolysekapazitäten
                for r in regions
                for h in hour
                for t in technologies if TagElectrolyzer[t] == 1)
