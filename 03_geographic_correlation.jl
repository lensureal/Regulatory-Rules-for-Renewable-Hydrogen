...
version = "_03_geographic_correlation"
...
UnmetDemandPenalty = readin("unmetdemandpenalty.csv", default=100, dims=2, dir=data_dir)
...
#### new constraint for termpaper ####

#nur erneuerbaren Strom verwenden für Wasserstoff
@constraint(ESM, RenewablePowerForHydrogen[y in year, h in hour, r in regions, f in fuels; f == "Power"],
...
@constraint(ESM, NewElectrolyzeRequiresNewRECapacity[y in year, r in regions, f in fuels; f == "Power"],
...
#Beschränkung des jährlichen Stromverbrauchs der neuen Elektrolysekapazitäten druch die in diesem Jahr verfügabaren Kapazitäten
@constraint(ESM, NewElectrolyzeRequiresNewRE[y in year, h in hour, r in regions, f in fuels; f == "Power"],
            sum(NewCapacity[y,r,t] * CapacityFactor[r,h,t] * OutputRatio[t,f] * 8760/n_hour
                for t in technologies if TagRenewable[t] == 1) >=
            sum(FuelUseByTechnology[y,r,h,t,f] - InputRatio[t,f] *
            (AccumulatedCapacity[y,r,t] - NewCapacity[y,r,t]) * CapacityFactor[r,h,t] #Stromverbrauch der neuen Elektrolysekapazitäten
                for t in technologies if TagElectrolyzer[t] == 1)* 8760/n_hour
